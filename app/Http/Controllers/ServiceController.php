...
/**
 * Calculate and store service provider earnings when order is completed 
 * FIXED: Customer pays tax, service provider only pays platform fees
 */
    ...
    try {
        ...
        switch($pricing_type) {
            ...
            case 'fixed':
                ...
                $duration_value = 1; // Fixed pricing is always 1 unit
                break;
        }

        // The gross amount for service provider is their base service amount
        ...
        // FIXED: Platform fees are deducted from service provider
        $platform_fee_amount = ($gross_amount * $platform_percentage / 100) + $platform_fee_fixed;

        // FIXED: Tax is NOT deducted from service provider - customer pays it
        // Tax amount is stored for record keeping but not deducted from provider earnings
        ...
        $tax_amount = 0;
        if ($tax) {
            // Tax amount is calculated for record keeping (customer paid this)
            $tax_amount = $base_service_amount * ($tax->percentage / 100);
        }

        // Coupon discount (platform absorbs this, not deducted from service provider)
        $coupon_discount = $order->discounted_amount ?? 0;

        // FIXED: Service provider net earnings = gross amount - ONLY platform fees
        // Tax is NOT deducted because customer pays it separately
        $net_earnings = $gross_amount - $platform_fee_amount;

        // Ensure net earnings don't go negative
        if ($net_earnings < 0) {
            $net_earnings = 0;
            Log::warning('Net earnings calculated as negative, set to 0', [
                'order_id' => $order_id,
        ...
        if ($pending_earning) {
            ...
            $pending_earning->update([
                'gross_amount' => $gross_amount,
                'platform_fee_amount' => $platform_fee_amount,
                'tax_amount' => $tax_amount, // Stored for record keeping (customer paid this)
        ...
        Log::info('Service earnings calculated - customer pays tax, provider pays platform fees', [
            'order_id' => $order_id,
            'service_provider_id' => $service_provider_id,
            'gross_amount' => $gross_amount,
            'platform_fee_amount' => $platform_fee_amount,
            'tax_amount' => $tax_amount, // Customer paid this
            'net_earnings' => $net_earnings
        ]);
        ...
        Log::error('Error calculating service earnings', [
            'order_id' => $order_id,
            'error' => $e->getMessage()
        ]);
        return false;
