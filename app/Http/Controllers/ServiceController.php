...
class ServiceController extends Controller
{
    ...
    /**
     * order_service - Updated to handle hourly, daily, and fixed pricing
     *
     * @return \Illuminate\Http\JsonResponse
     */
    ...
    {
        $validation = $request->validate([
            'duration_value' => 'numeric|required', // The duration input (hours, days, or 1 for fixed)
        ...
        switch ($service["pricing_type"]) {
            ...
            case 'fixed':
                ...
                // Ignore duration_value for fixed pricing
                break;
        ...
        if ($validation) {
            // Platform fees and taxes calculation (unchanged)
     ...
     * FIXED: Follows correct business rules - Platform earns from both customer and provider fees
     */
    private function calculateAndStoreEarnings($order_id)
    {
        try {
            ...
            // Get current platform fees settings (snapshot them for this order)
            $platform_fee_fixed = 0;
            $platform_fee_percentage = 0;

            $_platform_fee_fixed = AppSetting::where('key', 'platform_fees')->first();
            if ($_platform_fee_fixed) {
                $platform_fee_fixed = floatval($_platform_fee_fixed->value);
            }

            $_platform_fee_percentage = AppSetting::where('key', 'platform_fees_percentage')->first();
            if ($_platform_fee_percentage) {
                $platform_fee_percentage = floatval($_platform_fee_percentage->value);
            }

            // CORRECT PLATFORM EARNINGS CALCULATION
            // Following your business rules: Platform earns from BOTH customer and provider

            // 1. Customer Platform Fees
            $customer_fixed_fee = $platform_fee_fixed;
            $customer_percentage_fee = ($base_service_amount * $platform_fee_percentage) / 100;
            $customer_platform_fee = $customer_fixed_fee + $customer_percentage_fee;

            // 2. Provider Platform Fees  
            $provider_fixed_fee = $platform_fee_fixed;
            $provider_percentage_fee = ($base_service_amount * $platform_fee_percentage) / 100;
            $provider_platform_fee = $provider_fixed_fee + $provider_percentage_fee;

            // 3. Total Platform Earnings = Customer fees + Provider fees
            $platform_earnings_total = $customer_platform_fee + $provider_platform_fee;

            // Calculate what provider actually receives
            ...
            if ($pending_earning) {
                ...
                $pending_earning->update([
                    ...
                    'customer_platform_fee' => $customer_platform_fee,     // NEW: Customer fees
                    'provider_platform_fee' => $provider_platform_fee,     // NEW: Provider fees
                    'platform_earnings_total' => $platform_earnings_total, // NEW: Total platform earnings
                    'platform_fee_fixed' => $platform_fee_fixed,           // NEW: Snapshot of fixed fee
                    'platform_fee_percentage' => $platform_fee_percentage, // NEW: Snapshot of percentage
            ...
            Log::info('Platform earnings calculated correctly', [
                'order_id' => $order_id,
